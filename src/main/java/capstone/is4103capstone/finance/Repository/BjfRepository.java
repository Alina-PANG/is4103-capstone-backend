...
public interface BjfRepository extends JpaRepository<BJF,String> {
    ...
    @Query(value="select f.id,f.code,f.created_date_time,s.object_name as service, v.object_name as vendor, f.oa_id as assessment_form_id, " +
            ...
            " ON f.service_id=s.id and f.vendor_id=v.id and f.requester_id=e1.id and f.approver_id=e2.id and c.id=f.cost_center_id and f.is_deleted=0 and f.vendor_id=?1 and f.bjf_status=1 and f.purchase_order_number is null",nativeQuery = true)
