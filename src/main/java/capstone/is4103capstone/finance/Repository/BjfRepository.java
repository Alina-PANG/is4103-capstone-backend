...
public interface BjfRepository extends JpaRepository<BJF,String> {
    ...
    @Query(value="select f.id,f.code,f.created_date_time,s.object_name as service, v.object_name as vendor, " +
            "f.bjf_status as bjf_status, c.code as cost_center_code, f.bjf_type, " +
    ...
    @Query(value="select f.id,f.code,f.created_date_time,s.object_name as service, v.object_name as vendor, f.bjf_status as bjf_status, c.code as cost_center_code, f.bjf_type, concat(f.estimated_budget,' ',f.currency) as total_budget, e2.user_name as approver,e1.user_name as requester" +
