...
public interface SeatAdminMatchRepository extends JpaRepository<SeatAdminMatch, String> {
    ...
    @Query(value = "SELECT * FROM seat_admin_match s WHERE s.is_deleted=false AND s.hierarchy_id=?1", nativeQuery = true)
    Optional<SeatAdminMatch> findUndeletedByHierarchyId(String hierarchyId);

    @Query(value = "SELECT * FROM seat_admin_match s WHERE s.is_deleted=false AND s.seat_admin_id=?1", nativeQuery = true)
