...
public interface SeatUtilisationLogRepository extends JpaRepository<SeatUtilisationLog, String> {
    ...
    @Query(value = "SELECT * FROM seat_utilisation_log s WHERE s.business_entity_id=?1 AND " +
    ...
    @Query(value = "SELECT * FROM seat_utilisation_log s WHERE s.business_entity_id=?1 AND s.year=?2 AND " +
    ...
    @Query(value = "SELECT * FROM seat_utilisation_log s WHERE s.business_entity_id=?1 AND s.created_time>?2 AND " +
