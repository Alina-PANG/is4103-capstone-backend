...
public interface EmployeeOfficeWorkingScheduleRepository extends JpaRepository<EmployeeOfficeWorkingSchedule, String> {
    ...
    Optional<EmployeeOfficeWorkingSchedule> findUndeletedOneByEmployeeIdAndOfficeId(String employeeId, String officeId);

    @Query(value = "SELECT * FROM employee_office_working_schedule e WHERE e.is_deleted=false AND e.employee_id=?1", nativeQuery = true)
    List<EmployeeOfficeWorkingSchedule> findUndeletedOnesByEmployeeId(String employeeId);
}
